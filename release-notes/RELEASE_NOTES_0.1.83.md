...
9月12日移动端后续增补：Android、HarmonyOS 在线包现已包含个人词库方案选择修复，仍为 0.1.83；重新下载覆盖安装即可。Windows/Linux 保持当天快符增补包。

- 备份与恢复中的个人词库操作先选方案；修正方案名称以及拼音／形码族表头。
- 四个导入导出按钮采用两行两列布局，文件选择期间固定目标方案。
- 形码按码表分别保存，全拼／双拼共用标准全拼个人词库；完整词库跨不同形码表或拼音／形码族误导入会被拒绝。
- 保持模型与模型来源声明不变。鸿蒙离线 AppGallery `.app` 单独交付上架，不公开上传。
