...
[官方网站](https://maquan.app/) · [下载页面](https://maquan.app/download) · [GitHub Release 0.1.42](https://github.com/BG5ERH/MaQuanIME/releases/tag/v0.1.42) · [更新记录](https://maquan.app/changelog) · [隐私声明](https://maquan.app/privacy/)

当前公开稳定版：

- Windows：`0.1.42`
- Android / Linux：`0.1.41`
...
| 平台 | 获取方式 |
|---|---|
| Windows | [MaQuanIME 0.1.42 安装程序](https://maquan.app/download) |
...
```bash
sudo apt install ./MaQuanIME-Linux-IBus-0.1.41-amd64.deb
sudo reboot
```
...
```bash
sudo apt install ./MaQuanIME-Linux-Fcitx5-0.1.41-amd64.deb
sudo reboot
```
...
```bash
sudo dnf install ./MaQuanIME-Linux-IBus-0.1.41-1.x86_64.rpm
sudo reboot
```
...
```bash
sudo dnf install ./MaQuanIME-Linux-Fcitx5-0.1.41-1.x86_64.rpm
sudo reboot
```
