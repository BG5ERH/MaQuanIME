...
[官方网站](https://maquan.app/) · [下载页面](https://maquan.app/download) · [GitHub Release 0.1.53](https://github.com/BG5ERH/MaQuanIME/releases/tag/v0.1.53) · [更新记录](https://maquan.app/changelog) · [隐私声明](https://maquan.app/privacy/)

当前公开稳定版：

- Windows / Android / Linux：`0.1.53`
...
- 官方三码郑码专用物理键映射，兼容 `v.`、空格和斜杠起始的短码/词组规则
...
| 平台 | 获取方式 |
|---|---|
| Windows | [MaQuanIME 0.1.53 安装程序](https://maquan.app/download) |
| Android | [官网 APK 0.1.53](https://maquan.app/download) |
...
```bash
sudo apt install ./MaQuanIME-Linux-IBus-0.1.53-amd64.deb
sudo reboot
```
...
```bash
sudo apt install ./MaQuanIME-Linux-Fcitx5-0.1.53-amd64.deb
sudo reboot
```
...
```bash
sudo dnf install ./MaQuanIME-Linux-IBus-0.1.53-1.x86_64.rpm
sudo reboot
```
...
```bash
sudo dnf install ./MaQuanIME-Linux-Fcitx5-0.1.53-1.x86_64.rpm
sudo reboot
```
